...
import Ajv from 'ajv';
import schema from '../CinematicSequence/schema/cinematic-sequence.schema.json';
...
const styles = {
  container: {
    ...
    flex: 1,
    backgroundColor: 'var(--mosaic-window-body-bg)',
    overflow: 'hidden',
  },
  toolbar: {
    display: 'flex',
    alignItems: 'center',
    padding: '4px 8px',
    borderBottom: '1px solid var(--mosaic-border-color)',
  },
  mainArea: { display: 'flex', flex: 1, overflow: 'hidden' },
  trackList: {
    width: 250,
    borderRight: '1px solid var(--mosaic-border-color)',
    overflowY: 'auto',
  },
  timelineArea: {
    flex: 1,
...
export default class CinematicSequenceEditor extends React.Component {
  ...
  constructor(props) {
    ...
    // JSON Schema validator (AJV). If ajv is not installed, validation
    // will be skipped and a warning will be logged. Install with
    // `npm install ajv` to enable validation in the editor.
    try {
      this._ajv = new Ajv({ allErrors: true });
      this._validateSequence = this._ajv.compile(schema);
    } catch (e) {
      console.warn('AJV schema validator not available:', e);
      this._ajv = null;
      this._validateSequence = null;
    }
  }
  ...
  _saveTracks(tracks) {
    ...
    if (this._validateSequence) {
      const valid = this._validateSequence(doc);
      if (!valid) {
        const errors = this._validateSequence.errors || [];
        const msgs = errors
          .map(err => `${err.instancePath || '/'} ${err.message}`)
          .join('\n');
        console.error('Cinematic sequence validation failed:', errors);
        alert('Cinematic sequence is invalid:\n' + msgs);
  ...
  _addKeyframe = (trackId, time) => {
    const tracks = this.state.tracks.map(t => {
      if (t.id === trackId) {
        return {
          ...t,
          keyframes: [...t.keyframes, { time, value: null }].sort(
            (a, b) => a.time - b.time
          ),
        };
      }
            ...
            {({ i18n }) => (
              <div style={styles.container}>
                ...
                <div style={styles.toolbar}>
                  <IconButton
                    onClick={this._resetPlayhead}
                    tooltip={i18n._(t`Rewind to start`)}
                  >
                    <FastRewind />
                  </IconButton>
                  <IconButton
                    onClick={isPlaying ? this._pause : this._play}
                    tooltip={isPlaying ? i18n._(t`Pause`) : i18n._(t`Play`)}
                  >
                ...
                <div style={styles.mainArea}>
                  ...
                  <div style={styles.timelineArea} ref={this._timelineRef}>
                    <div
                      style={{
                        ...styles.timeRulerContainer,
                        width: timelineWidth,
                      }}
                    >
                      <div
                        style={styles.timeRuler}
                      ...
                      {tracks.map(track => (
                        <div
                          key={track.id}
                          style={{ ...styles.trackRow, width: timelineWidth }}
                          onDoubleClick={e => {
